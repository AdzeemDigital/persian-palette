...
export interface LegacyGeoSpatial {
  ...
  coordinates: { lat: number; lng: number; altitudeMeters?: number };
}
export interface ColorScienceEvidence {
  oklab: { L: number; a: number; b: number }; srgb: { r: number; g: number; b: number };
  apca: { contrastOnWhite: number; contrastOnBlack: number; recommendedWeight: string };
}
...
export interface LegacyColorEvidence {
  historicalContextFa: string; artMovement: string; chemical: ChemicalEvidence;
  geo: GeoSpatialEvidence; physics: PhysicsEvidence; mineralogical: MineralogicalEvidence;
  geoSpatial: LegacyGeoSpatial; spectral: SpectralEvidence; colorScience: ColorScienceEvidence;
}
export interface ColorEvidence extends LegacyColorEvidence { provenance: EvidenceProvenance }
