 ...
 */
import { hexToRgb, rgbToHex, rgbToOklab, oklabToRgb } from './oklab.js';
/**
 ...
 */
export function mixHistoricalPigments(hex1, hex2, ratio = 0.5) {
    if (!Number.isFinite(ratio) || ratio < 0 || ratio > 1)
        throw new RangeError('ratio is the weight of the second color, in [0, 1]');
 ...
 */
export function generatePigmentGradient(hex1, hex2, steps = 5) {
    if (!Number.isInteger(steps) || steps < 1 || steps > 1000)
        throw new RangeError('steps must be an integer in [1, 1000]');
    const results = [];
